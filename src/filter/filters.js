import { RGBtoHSV, HSVtoRGB } from './util'
...
export function sepia (pixels, adj) {
  let d = pixels.data
  for (let i = 0; i < d.length; i += 4) {
    let r = d[i]
    let g = d[i + 1]
...
// No adjustment
export function invert (pixels, adj) {
  let d = pixels.data
  for (let i = 0; i < d.length; i += 4) {
    d[i] = 255 - d[i]
    d[i + 1] = 255 - d[i + 1]
...
export function brightness (pixels, adj) {
  let d = pixels.data
  adj = (adj > 1) ? 1 : adj
  adj = (adj < -1) ? -1 : adj
  adj = ~~(255 * adj)
...
export function hueSaturation (pixels, adj) {
  let d = pixels.data
  for (let i = 0; i < d.length; i += 4) {
...
export function saturation (pixels, adj) {
  let d = pixels.data
  adj = (adj < -1) ? -1 : adj
  for (let i = 0; i < d.length; i += 4) {
    let r = d[i]
...
export function contrast (pixels, adj) {
  adj *= 255
  let d = pixels.data
...
export function colorFilter (pixels, rgbColor) {
  let d = pixels.data
  let adj = rgbColor[3]
  for (let i = 0; i < d.length; i += 4) {
...
// RGB Adjust
export function rgbAdjust (pixels, rgbAdj) {
  let d = pixels.data
...
export function convolute (pixels, weights) {
